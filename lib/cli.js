...
var makeCommands = function () {
  var commands = [
    Command('start', function (program) {
      program
        .description('Start the binder-registry server')
        .option('-a, --api-key <key>', 'Binder API key')
        .option('-c, --config <file>', 'Module configuration file')
        .option('-p, --port <port>', 'the binder-registry server port')
      return program
    }, function (options) {
      console.log('Starting the binder-registry server...')
      var opts = {}
      if (options.config) {
    ...
    Command('stop', function (program) {
      program
        .description('Stop the binder-registry server')
      return program
    }, function (options) {
      console.log('Stopping the binder-registry server...')
      shell.exec('pm2 delete binder-registry-start')
    })
