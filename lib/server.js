...
BinderRegistry.prototype.storeTemplate = function (req, res) {
  var templateJson = req.body
  var conditions = { name: templateJson.name }
  this.templateModel.findOneAndUpdate(conditions, templateJson, { upsert: true }, function (err, obj) {
    if (err) {
      console.error(err)
      return res.status(500).send('Could not store template in database')
    }
    res.json({
...
BinderRegistry.prototype.getTemplate = function (req, res) {
  ...
  this.templateModel.findOne({ name: templateName }, function (err, template) {
    if (err) {
      console.error(err)
      return res.status(500).send('Could not search for template')
    }
    template = template || {}
    res.json(template)
  })
